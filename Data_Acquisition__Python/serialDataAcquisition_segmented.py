    ...
    try:
        ...
        if ((len(data) % segmentLength) == 0) and (recordData == True) and (interruptToken == False):
            if recordData == True:
                ...
                f_s = 1 / np.diff(T)
                f_s = np.delete(f_s, 0)
                print('\nSampling Frequency:\t\tMean = {:.2f}Hz  |  Max = {:.2f}Hz  |  Min = {:.2f}Hz'.format(f_s.mean(), f_s.max(),f_s.min()))
            if count >= recorditeration:
                df = pd.DataFrame(data,columns=columnName)
                df.to_csv(storePath + '/' + gestureName + '.csv', index=False)
                print("\nData Writing Done... shape: ", df.shape)
                break
